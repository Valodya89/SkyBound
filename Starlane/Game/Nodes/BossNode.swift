...
/// The warden hovering on the horizon. Cracks appear as it loses health.
///
/// Geometry is built once per layout, centred on the node's origin; each frame only moves the node, so no
/// paths or shape nodes are allocated while a boss is on screen.
final class BossNode: SKNode {
    ...
    private var scarLines: [SKShapeNode] = []
    ...
    private func build(width W: Double) {
        builtWidth = W
        let hp = CGMutablePath()
        hp.move(to: CGPoint(x: 0, y: W * 0.09))
        hp.addLine(to: CGPoint(x: W * 0.18, y: 0))
        hp.addLine(to: CGPoint(x: W * 0.10, y: -W * 0.065))
        hp.addLine(to: CGPoint(x: -W * 0.10, y: -W * 0.065))
        hp.addLine(to: CGPoint(x: -W * 0.18, y: 0))
        hp.closeSubpath()
        hull.path = hp
        let fp = CGMutablePath()
        fp.move(to: CGPoint(x: 0, y: W * 0.09))
        fp.addLine(to: CGPoint(x: W * 0.18, y: 0))
        fp.addLine(to: CGPoint(x: 0, y: W * 0.02))
        fp.closeSubpath()
        facet.path = fp
        for (i, l) in scarLines.enumerated() {
            let p = CGMutablePath()
            p.move(to: CGPoint(x: -W * 0.12 + Double(i) * W * 0.08, y: W * 0.05))
            p.addLine(to: CGPoint(x: -W * 0.09 + Double(i) * W * 0.08, y: -W * 0.05))
            l.path = p
        }
    ...
    func update(boss: BossState?, proj: Projector, sceneHeight: CGFloat, time: TimeInterval) {
        ...
        if builtWidth != W { build(width: W) }
        let bob = sin(time * 1000 / 430) * W * 0.09
        let bx = W / 2 + bob
        let by = sceneHeight - (HZ - H * 0.055)

        for n in [hull, facet, scars] as [SKNode] { n.position = CGPoint(x: bx, y: by) }
...
final class GhostNode: SKNode {
    ...
    private var pathRadius: Double = -1
    ...
    /// The outline is rebuilt only when its size changes by a whole point; otherwise the path is reused.
    private func rebuildPath(radius r: Double) {
        guard r != pathRadius else { return }
        pathRadius = r
    ...
    func update(hint: PathHint?, proj: Projector, sceneHeight: CGFloat) {
        ...
        rebuildPath(radius: (r * 2).rounded() / 2)
