            ...
            VStack(spacing: 0) {
                ScrollView(showsIndicators: false) {
                    VStack(alignment: .leading, spacing: 18) {
                        ...
                        .panel()
                        HStack(spacing: 8) {
                            bankChip { CoinIcon(size: 13); Text("+\(GameFormat.grouped(outcome?.coinsEarned ?? run.simulation.coins))").display(17).monospacedDigit() }
                ...
                VStack(spacing: 8) {
                    ...
                    let canRelaunch = mode == .daily || EnergySystem.canLaunch(mode, profile: p)
                    Button {
                        if canRelaunch { coordinator.playAgain() } else { coordinator.returnToHub() }
                    } label: {
                        HStack(spacing: 6) {
                            Text(mode == .daily ? "Back to deck" : canRelaunch ? "Launch again" : "Out of energy")
                            Spacer()
                            if mode != .daily {
                                HStack(spacing: 6) {
                                    Text(canRelaunch ? "\(mode.shortName) · \(mode.config.energyCost)" : "Back to deck")
                                    if canRelaunch { EnergyIcon(size: 12, tint: Theme.flareInk) }
                                }
