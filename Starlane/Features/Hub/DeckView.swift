    ...
    private var readouts: some View {
        ...
        return HStack(alignment: .top) {
            ...
            VStack(alignment: .trailing, spacing: 6) {
                Text("Best · \(mode.shortName)").capsLabel(color: .white.opacity(0.7))
        ...
        HStack(spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                HStack(alignment: .firstTextBaseline, spacing: 8) {
                    Text(p.displayName).font(.body(14, weight: .bold)).foregroundStyle(Theme.text).lineLimit(1).minimumScaleFactor(0.8)
                    Text("LVL \(p.level)").capsLabel(color: Theme.flare)
                    if p.isVIP { LineIcon(name: "crown", size: 12, color: Theme.gold, weight: .bold) }
                }
                HStack(spacing: 6) {
                    ProgressBar(fraction: Progression.fraction(for: p), color: Theme.ice, height: 3).frame(width: 44)
...
struct LaunchDeck: View {
    ...
    private func energyRow(_ p: PlayerProfile) -> some View {
        HStack(spacing: 10) {
            ...
            SegmentBar(filled: p.energy, total: p.maxEnergy, height: 8, segmentWidth: p.maxEnergy > 6 ? 12 : 22)
...
struct DailyStrip: View {
    ...
    var body: some View {
        ...
        let rewardText = reward.skinID != nil ? "Claim rocket" : reward.gems > 0 ? "Claim \(reward.gems) gems" : "Claim \(reward.coins) coins"
        HStack(spacing: 8) {
            cell(icon: "calendar", kicker: "Day \(DailySystem.displayDay(p))", value: p.loginClaimedToday ? "Claimed" : rewardText, dot: !p.loginClaimedToday) {
        ...
        Button {
            ...
            HStack(spacing: 8) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text(value).font(.body(12, weight: .semibold)).foregroundStyle(Theme.text).monospacedDigit().lineLimit(1).minimumScaleFactor(0.7)
                }
                .frame(maxWidth: .infinity, alignment: .leading)
            }
