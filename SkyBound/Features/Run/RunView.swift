        ...
        VStack(alignment: .trailing, spacing: 6) {
            ...
            if hud.magnetSeconds > 0 {
                chip {
                    ...
                    ProgressBar(fraction: Double(hud.magnetSeconds) / 8, color: Theme.flare, height: 4, track: .white.opacity(0.18)).frame(width: 36)
                }
            }
...
struct SectorBanner: View {
    ...
    var body: some View {
        VStack(spacing: 8) {
            ...
            Text(biome.name).display(44, color: Color(hex: "#FFE1C2")).shadow(color: .black.opacity(0.4), radius: 0, y: 3)
        }
        .frame(maxWidth: .infinity)
        .opacity(phase)
        .onAppear {
            withAnimation(.easeOut(duration: 0.45)) { phase = 1 }
            withAnimation(.easeIn(duration: 0.6).delay(2.1)) { phase = 0 }
        }
        .id(biome.id)
    }
}
