            ...
            GameScreen("Daily", kicker: "Resets in \(resetText)") {
                ...
                HStack(spacing: 5) {
                    ForEach(DailyLoginReward.calendar) { r in
                        StreakDay(reward: r, state: r.day < p.loginStreakDay ? .past : (r.day == p.loginStreakDay && !p.loginClaimedToday) ? .today : .future)
                    }
                }
                Button { coordinator.claimDailyLogin() } label: {
                    HStack {
                        Text(p.loginClaimedToday ? "Claimed · back tomorrow" : "Claim day \(p.loginStreakDay)")
                        Spacer()
