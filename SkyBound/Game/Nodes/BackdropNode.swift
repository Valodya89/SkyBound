...
final class BackdropNode: SKNode {
    ...
    func layout(size: CGSize, projector: Projector) {
        ...
        ground.size = CGSize(width: W + 60, height: H - HZ + 30)
        ...
        stars = (0..<90).map { _ in
            ...
            n.position = CGPoint(x: x, y: skY(rng.next(in: 0...(HZ * 1.04))))
    ...
    func apply(biome next: Biome, animated: Bool) {
        ...
        let groundTex = textures.verticalGradient(key: "ground.\(next.id)", stops: [(next.groundBottom, 0), (next.groundTop, 1)])
        ...
        horizonLine.color = SKColor(next.edge)
        horizonLine.alpha = 0.7
