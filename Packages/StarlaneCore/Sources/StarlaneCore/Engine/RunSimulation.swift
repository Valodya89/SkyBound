...
public struct RunSimulation: Sendable {
    ...
    /// Hazards (lightning, gusts) roll on their own stream so the number of `step` calls never changes the course.
    private var hazardRNG: any RandomSource
    ...
    /// `speed` before the slow-mo factor; chunk spacing is authored against this so slow-mo never packs rows.
    public private(set) var cruiseSpeed: Double = 0
    ...
    public init(config: RunConfig) {
        ...
        if let seed = config.seed {
            ...
            hazardRNG = Mulberry32(seed: seed &+ 0x9E37_79B9)
        } else {
            rng = SystemRandomSource()
            hazardRNG = SystemRandomSource()
        }
    ...
    /// The 2× bonus itself is credited by the caller; keeping `coins` untouched means a run that is revived
    /// afterwards banks only what was actually collected.
    /// Lets shake, flash and strobe settle while the world is frozen (countdown, pause, results).
    public mutating func decayEffects(dt rawDT: Double) {
    ...
    private mutating func updatePathHint() {
        ...
        let lockedLaneBlocked = hintLane.map { l in
            entities.contains { ($0.kind == .wall || $0.kind == .laser) && $0.lane == l && $0.z > -10 && $0.z < look }
        } ?? false
        if hintThreatID != threat.id || lockedLaneBlocked {
    ...
    public mutating func step(dt rawDT: Double) -> [RunEvent] {
        ...
        if isRunning { stepHazards(dt: dt, events: &events) }
        ...
        if isRunning {
            ...
            cruiseSpeed = min(unclamped, baseSpeed * Self.maxSpeedMultiplier)
            speed = cruiseSpeed * slow
            let mv = speed * dt * 2.6
            worldZ += mv
        ...
        let travel = max(speed, laserSpeed) * dt * 2.6
        ...
        if isRunning { collide(events: &events, travel: travel) }
        ...
        for i in entities.indices where !entities[i].hasPassed && !entities[i].isDead {
            ...
            // A dodged laser wounds the boss, but only while the player is actually flying.
            if isRunning, entities[i].kind == .laser, boss != nil {
    ...
    private mutating func stepHazards(dt: Double, events: inout [RunEvent]) {
        ...
        case .gust:
            gustTimer -= dt
            if gustTimer <= 0 {
                gustTimer = hazardRNG.next(in: 150...300)
                gustDirection = hazardRNG.chance(0.5) ? -1 : 1
            }
    ...
    private mutating func emitChunk() {
        ...
        let stretch = Self.spacingStretch(speed: max(cruiseSpeed, baseSpeed * Self.rampStartFraction), difficulty: difficulty)
    ...
    private mutating func stepBoss(dt: Double, events: inout [RunEvent]) {
        ...
        if b.telegraph > 0 {
            b.telegraph -= dt
            if b.telegraph <= 0 {
                entities.append(makeEntity(.laser, lane: b.telegraphLane, z: Self.laserSpawnDepth, bossLaser: true))
    ...
    /// Depth at which boss lasers appear.
    static let laserSpawnDepth = Projector.maxDepth * 0.72

    /// Picks a lane for the next laser that does not already carry a wall or laser near the spawn depth, so the
    /// tail of the last chunk and the shot can never seal all three lanes at once.
    private mutating func laserLane() -> Int {
        let depth = Self.laserSpawnDepth
        let free = (0..<3).filter { lane in
            !entities.contains { ($0.kind == .wall || $0.kind == .laser) && $0.lane == lane && abs($0.z - depth) < 160 }
        }
        guard !free.isEmpty else { return rng.nextIndex(count: 3) }
        return free[rng.nextIndex(count: free.count)]
    }
    ...
    /// `travel` is how far entities moved this step; the window is swept by it so a long frame at top speed
    /// cannot carry an obstacle straight through the ship.
    private mutating func collide(events: inout [RunEvent], travel: Double) {
        for i in entities.indices {
            let e = entities[i]
            if e.isDead || e.hasPassed { continue }
            if e.z > 34 || e.z < -46 - travel { continue }
            ...
            case .coin:
                ...
                let multiplier = Double(combo) * mode.coinRate * (1 + Double(upgradeLevel(.coinValue)) * 0.15) * (doubleCoins ? 2 : 1) * (config.isVIP ? 2 : 1)
    ...
    private mutating func attractStep(dt: Double) -> [RunEvent] {
        ...
        spawnCursor -= mv
        if spawnCursor <= 0 {
            // Same row spacing a real run uses at this speed, so consecutive hurdles are actually hoppable.
            let stretch = Self.spacingStretch(speed: 4.2, difficulty: 0.5)
            let chunk = ChunkLibrary.all[rng.nextIndex(count: ChunkLibrary.all.count)]
            for item in chunk.items {
                entities.append(makeEntity(item.kind, lane: item.lane, z: Projector.maxDepth + item.z * stretch,
                                           boost: item.kind == .orb ? .shield : nil))
            }
            spawnCursor = chunk.length * stretch
        }
        for i in entities.indices {
        ...
        // React to the *nearest* obstacle in the lane, timed so the hop or dive is centred on it.
        let perFrame = 4.2 * 2.6
        let nearest = entities
            .filter { $0.lane == ship.targetLane && $0.kind.isObstacle && $0.z >= 20 && $0.z <= 720 }
            .min { $0.z < $1.z }
        var need: EntityKind? = nil
        if let n = nearest {
            switch n.kind {
            case .low where n.z <= perFrame * 14 + 34: need = .low
            case .beam where n.z <= perFrame * 12 + 34: need = .beam
            case .wall where n.z <= 430: need = .wall
            default: break
            }
        }
