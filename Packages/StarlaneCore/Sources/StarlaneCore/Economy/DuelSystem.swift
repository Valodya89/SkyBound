...
public enum DuelSystem {
    ...
    /// Applies a finished run to every open duel. Returns the duels that were just won. A revived
    /// continuation of a run that was already counted passes `consumesAttempt: false`.
    @discardableResult
    public static func resolve(score: Int, consumesAttempt: Bool = true, profile: inout PlayerProfile) -> [Duel] {
        var won: [Duel] = []
        for i in profile.duels.indices where profile.duels[i].status == .open {
            if consumesAttempt { profile.duels[i].triesLeft -= 1 }
        ...
        let closed = profile.duels.filter { $0.status != .open }
        if closed.count > keptHistory {
            let drop = Set(closed.dropLast(keptHistory).map(\.id))
            profile.duels.removeAll { drop.contains($0.id) }
        }
        return won
    }

    /// Finished duels kept for the history list.
