...
public enum EnergySystem {
    /// Cap a player can reach by buying slots (VIP adds its own +2 on top).
    public static let maxCap = 12
    public static let baseCap = 5
    public static var purchasableSlots: Int { maxCap - baseCap }
    public static let refillPrice = Price.gems(15)

    public static func canRaiseCap(_ profile: PlayerProfile) -> Bool { profile.energySlotsBought < purchasableSlots }
    ...
    public static func refill(_ profile: inout PlayerProfile) -> Bool {
        guard profile.energy < profile.maxEnergy, profile.spend(refillPrice) else { return false }
    ...
    public static func raiseCap(_ profile: inout PlayerProfile) -> Bool {
        guard canRaiseCap(profile), profile.spend(capUpgradePrice(for: profile)) else { return false }
        profile.maxEnergy += 1
