...
public enum DailySystem {
    ...
    public static func rollover(_ profile: inout PlayerProfile, now: Date, calendar: Calendar = .current) -> Bool {
        ...
        // Only a *later* day counts; a device clock wound backwards must not hand out the dailies again.
        guard profile.lastActiveDay == 0 || today > profile.lastActiveDay else { return false }
    ...
    /// The calendar day to show: the day just claimed while today's claim is done, otherwise the day to claim.
    public static func displayDay(_ profile: PlayerProfile) -> Int {
        guard profile.loginClaimedToday else { return profile.loginStreakDay }
        return profile.loginStreakDay == 1 ? DailyLoginReward.calendar.count : profile.loginStreakDay - 1
    }
