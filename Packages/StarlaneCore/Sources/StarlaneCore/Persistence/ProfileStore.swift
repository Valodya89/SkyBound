...
public final class FileProfileStore: ProfileStore {
    ...
    /// Default location: `<Application Support>/Starlane/profile.json`.
    public static func standard(fileManager: FileManager = .default) throws -> FileProfileStore {
        let base = try fileManager.url(for: .applicationSupportDirectory, in: .userDomainMask, appropriateFor: nil, create: true)
        let dir = base.appendingPathComponent("Starlane", isDirectory: true)
        try fileManager.createDirectory(at: dir, withIntermediateDirectories: true)
        return FileProfileStore(fileURL: dir.appendingPathComponent("profile.json"))
    }

    /// A file that exists but cannot be decoded is moved aside (`profile.corrupt-<unix time>.json`) before the
    /// error is rethrown, so a fresh profile never overwrites the only copy of a player's progress.
    public func load() throws -> PlayerProfile? {
        ...
        do {
            ...
            let aside = fileURL.deletingLastPathComponent().appendingPathComponent("profile.corrupt-\(stamp).json")
